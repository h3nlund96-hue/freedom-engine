-- ── Build Log — #032 through #038 ───────────────────────────────────────────
-- Run this in the Supabase SQL Editor for your project.
-- Backfills the Build history for everything shipped this session, nested
-- under "Quest 2 — Build Freedom Engine Alpha" (same Quest as #004–#031),
-- continuing on from whatever sort_order already exists there.
--
...
from (
  select
    'Build #032 — Idea Vault: Title, Description & Status' as title,
  ...
  union all
  select
    'Build #033 — Idea Vault: Detail Modal & Convert to Quest',
  ...
  union all
  select
    'Build #034 — Quest Board: Full CRUD & Active Focus Panel',
  ...
  union all
  select
    'Build #035 — Visual Consistency for Side Quests & Completed',
  ...
  union all
  select
    'Build #036 — Active Path Exclusivity Fix',
  ...
  union all
  select
    'Build #037 — Quest Board Retab: Active Path, Quest Lines, Quests, Side Quests',
  ...
  union all
  select
    'Build #038 — Remove Questline Active Status & Quest Reassignment',
