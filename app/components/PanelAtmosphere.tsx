...
export function PanelAtmosphere({ bias }: PanelAtmosphereProps) {
  const background =
    bias === "cyan"
      ? "radial-gradient(ellipse 80% 60% at 20% 15%, color-mix(in srgb, var(--accent-glow) var(--panel-atmosphere-mix), transparent) 0%, transparent 70%)"
      : bias === "amber-cyan"
      ? "radial-gradient(ellipse 70% 55% at 15% 10%, color-mix(in srgb, var(--accent) var(--panel-atmosphere-mix), transparent) 0%, transparent 65%), radial-gradient(ellipse 60% 50% at 90% 90%, color-mix(in srgb, var(--accent-glow) var(--panel-atmosphere-mix), transparent) 0%, transparent 70%)"
      : "radial-gradient(ellipse 80% 60% at 20% 15%, color-mix(in srgb, var(--accent) var(--panel-atmosphere-mix), transparent) 0%, transparent 70%)";
